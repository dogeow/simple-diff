...
describe('HistoryPage labels', () => {
  ...
  it('filters history entries by the same host and directory combination', async () => {
    ...
    const filter = await screen.findByLabelText('对比组合')
    await user.selectOptions(filter, 'sftp:47.99.220.36:/var/www/dogeow-api-next↔sftp:23.144.116.91:/var/www/dogeow-api')

    expect(screen.getByText('当前 2 条')).toBeTruthy()
    expect(screen.getAllByTitle('DogeOW:/var/www/dogeow-api-next ↔ Hermes:/var/www/dogeow-api')).toHaveLength(2)
    expect(screen.queryByTitle('DogeOW:/srv/api ↔ Hermes:/var/www/dogeow-api')).toBeNull()
  })
})